...
export const SupportMessageModal = (props) => {
  const users = JSON.parse(localStorage.getItem('doc-user'))
  const [user, setUser] = useState(null)
  const {setOpen,open,} = props
  const [data,setData] = useState({message:'',title:""})
  const [isError, setIsError] = useState('')
...
// console.log(users);
  ...
  return (
    <ModalWrapper isOpen={()=>setOpen(!open)}>
      <ModalContent>
        <ModalHeader>
          <ModalTitle>Support Team</ModalTitle>
        </ModalHeader>
        <ModalBody>
          <Group>
            <Label>Title</Label>
            <Input placeholder='Enter Title' type={'text'} onChange={(e)=>setData({...data,title:e.target.value})}/>
          </Group>
          <Group>
            <Label>Message</Label>
            <TextArea rows={6} placeholder='Enter Message' type={'text'} onChange={(e)=>setData({...data,message:e.target.value})}/>
          </Group>
          {isError && <Error>{isError}</Error>}
        </ModalBody>
        <ModalFooter>
          <ModalButton secondary onClick={()=>setOpen(false)}>Cancel</ModalButton>
          <ModalButton primary onClick={handleConfirm}>Send</ModalButton>
        </ModalFooter>
      </ModalContent>
    </ModalWrapper>
  );
...
// console.log(users);
   const handleConfirm = async() => {
    setIsError('')
    await base_url.delete(`delete_users/${user?.id}/`,{
        'headers':{
            'Content-Type':'multipart/form-data'
        }
    }).then((res)=>{
    ...
    <ModalWrapper isOpen={()=>setOpen(!open)}>
      <ModalContent>
        ...
        <ModalBody style={{border:'none'}}>
          <Group>
           <Label>Are sure want to delete your acount </Label>
          </Group>
          {isError && <Error>{isError}</Error>}
        </ModalBody>
        <ModalFooter>
          <ModalButton secondary onClick={()=>setOpen(false)}>Cancel</ModalButton>
          <ModalButton primary onClick={handleConfirm}>Delete</ModalButton>
        </ModalFooter>
      </ModalContent>
    </ModalWrapper>
  );
