...
import axios from 'axios';
 import { ConversionForm, DownloadButton, DownloadWrapper, FileInput, FileInputContainer, FileTitle, FormTitle,
  PdfToWordPageContainer, SubmitButton, UploadIcon, UploadText, Error,RefreshButton,RefreshIcon,
...
function PptToPdf() {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop: onDrop,
    accept: {
      'application/ppt': ['ppt','.ptx'],
      'application/vnd.ms-powerpoint': ['ppt','.ptx'],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true)
      setIsError('')
      await base_url.post('ppt_to_pdf/'+user.id+'/', formData, {
        headers: {
        ...
        {!convertedFile && <>
          {!data.file && <>
           <Dropzone {...getRootProps()} >
              <FormTitle>Drag 'n' drop a PPT file here</FormTitle>
              <FileInputContainer>
          ...
          {data.file && 
            <Container>
              <Content>
                  ...
                  <PreviewContainer>
                    <Preview>
                      <PreviewImage src={require('../../Assets/Images/ppt.png')} alt={`Page`} />
                      <FileName>{data.file.name}</FileName>
                    </Preview>
                  </PreviewContainer>
              </Content>
              <Form onSubmit={handleSubmit}>
                <FileTitle>PPT to PDF</FileTitle>
                <Spacer></Spacer>
                {/* <Group>
