...
function WordToPdf() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true)
      setIsError('')
      await base_url.post('word_to_pdf/'+user.id+'/', formData, {
        headers: {
        ...
        {!convertedFile && <>
          {!data.file && <>
           <Dropzone {...getRootProps()} >
              <FormTitle>Drag 'n' drop a Word file here</FormTitle>
              <FileInputContainer>
          ...
          {data.file && 
            <Container>
              <Content>
                  ...
                  <PreviewContainer>
                    <Preview>
                      <PreviewImage src={require('../../Assets/Images/word.png')} alt={`Page`} />
                      <FileName>{data.file.name}</FileName>
                    </Preview>
                  </PreviewContainer>
              </Content>
              <Form onSubmit={handleSubmit}>
                <FileTitle>Word to PDF</FileTitle>
                <Spacer></Spacer>
                {/* <Group>
