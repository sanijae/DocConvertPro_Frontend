...
function ExtractPdf() {
  const [data, setData] = useState({ file: null, start: '' , end: ''});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true)
      setIsError('')
      await base_url.post('extract_pdf/'+user.id+'/', formData, {
        headers: {
          ...
          {data.file &&  
            <Container>
              ...
              <Form onSubmit={handleSubmit}>
                <FileTitle>Extract PDF</FileTitle>
                <Spacer></Spacer>
                <Group>
                  <Label>Start from</Label>
                  <Input type='number' placeholder=' 1' onChange={(e)=>setData({...data, start:e.target.value})} value={data.start} />
                </Group>
                <Group>
                  <Label>End</Label>
                  <Input type='number' placeholder=' 10' onChange={(e)=>setData({...data, end:e.target.value})} value={data.end} />
                </Group>
                {isError && <Error>{isError}</Error>}
                <SubmitButton type="submit">Extract</SubmitButton>
              </Form>}
            </Container>}
          </>}
          {isError && <Error>{isError}</Error>}
