...
import { ConversionForm, DownloadButton, DownloadWrapper, FileInput, FileInputContainer,
   FileTitle, FormTitle,PdfToWordPageContainer, SubmitButton, UploadIcon, UploadText, 
   Error,RefreshButton,RefreshIcon,Container,Content,Form,Spacer,PreviewContainer, Dropzone,
   PreviewImage,Preview,AddContainer,AddButton,AddIcon,IconWrapper,FileInputLabel} from './Styles'
...
function RepairPdf() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true)
      setIsError('')
      await base_url.post('repair_pdf/'+user.id+'/', formData, {
        headers: {
       ...
       setIsError('')
        ...
        setIsError(err.data.response.error)
          ...
          {data.file && 
            <Container>
              ...
              <Form onSubmit={handleSubmit}>
                <FileTitle>Repair PDF</FileTitle>
                <Spacer></Spacer>
                {isError && <Error>{isError}</Error>}
                {/* <Group>
                  <Label>Rotation angle</Label>
                  <Input type='password' placeholder=' 90 for 90degree' onChange={(e)=>setData({...data, angle:e.target.value})} value={data.angle} />
                </Group> */}
                <SubmitButton type="submit">Repair</SubmitButton>
              </Form>}
            </Container>}
          </>}
          {convertedFile && (
