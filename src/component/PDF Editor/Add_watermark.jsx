...
import axios from 'axios';
 import { ConversionForm, DownloadButton, DownloadWrapper, FileInput, FileInputContainer, FileTitle, FormTitle,
  ...
  Spacer,PreviewContainer, Dropzone,PreviewImage,Preview,AddContainer,AddButton,AddIcon,IconWrapper,FileInputLabel,
...
function AddWatermark() {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('watermark',data[1])
    
    try {
      setIsLoading(true)
      setIsError('')
      await base_url.post('add_watermark/'+user.id+'/', formData, {
        headers: {
          ...
          {data.length && 
            <Container>
             ...
             {images.length && <Form onSubmit={handleSubmit}>
                <FileTitle> Add watermark</FileTitle>
                <Spacer></Spacer>
                {/* <Group>
                  <Label>Add a pdf</Label>
                  <Input type='file' onChange={handlePdfChange}  />
                </Group> */}
                {data.length >2 && <Error>only two files are allow</Error>}
                {isError && <Error>{isError}</Error>}
                {data.length > 1 ? (
                  <SubmitButton type="submit"> Watermark</SubmitButton>
                 ) : (
                  <DisableButton type="submit" disabled>Add Watermark</DisableButton>
                )}
              </Form>}
            </Container>}
          </>}
