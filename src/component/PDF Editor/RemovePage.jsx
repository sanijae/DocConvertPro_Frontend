...
import axios from 'axios';
 import { ConversionForm, DownloadButton, DownloadWrapper, FileInput, FileInputContainer, FileTitle, FormTitle,
  ...
  Content,Form,Spacer,PreviewContainer, Dropzone,PreviewImage,Preview,AddContainer,AddButton,AddIcon,
  IconWrapper,FileInputLabel,Group,Label,Input} from './Styles'
...
function RemovePage() {
  const [data, setData] = useState({ file: null, page: []});
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('page', data.page.map(page => parseInt(page)));
    console.log(data.page.map(page => parseInt(page)));
    // data.page.forEach(page => {
    //   formData.append('page', page);
    // });
    // data.page.split(',').map(page =>formData.append('page', parseInt(page)))
    // formData.append('page', data.page.split(',').map(page => parseInt(page)));
    try {
      setIsLoading(true)
      setIsError('')
      await base_url.post('remove_page/'+user.id+'/', formData, {
        headers: {
      ...
      setIsError('Error while processing pdf file')
      setIsLoading(false)
    }
  }
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Group>
                  ...
                  <Input 
                    type='text' 
                    placeholder='1,2,4' 
                    onChange={(e) => setData({...data, page: e.target.value.split(',')})} 
                    value={data.page.join(',')} 
                  />
                </Group>
                {isError && <Error>{isError}</Error>}
                <SubmitButton type="submit">Remove page</SubmitButton>
              </Form>}
            </Container>}
          </>}
          {convertedFile && (
